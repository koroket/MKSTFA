...
@implementation DraggableBackground
{
	...
	NSMutableArray *loadedCards; //%%% the array of card loaded (change max_buffer_size to increase or decrease the number
	                             //of cards this holds)
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		[self setupView];
		exampleCardLabels =
		    [[NSUserDefaults standardUserDefaults] arrayForKey:@"places"]; //%%% placeholder for card-specific information
...
- (Draggable *)createDraggableWithDataAtIndex:(NSInteger)index {
	Draggable *draggable = [[Draggable alloc]
	                        initWithFrame:CGRectMake((self.frame.size.width - CARD_WIDTH) / 2,
...
- (void)loadCards {
	if ([exampleCardLabels count] > 0) {
		NSInteger numLoadedCardsCap =
		    (([exampleCardLabels count] > MAX_BUFFER_SIZE) ? MAX_BUFFER_SIZE : [exampleCardLabels count]);
		//%%% if the buffer size is greater than the data size, there will be an array error, so this makes sure that
		//doesn't happen

		//%%% loops through the exampleCardsLabels array to create a card for each label.  This should be customized by
		//removing "exampleCardLabels" with your own array of data
